...
public final class BmForms {
     ...
     *
     * <p>Denodo hands the archival date columns back as ISO text — they are
     * typed Date/Timestamp in the views even though the workbook documents the
     * format as YYYYMMDD — so ISO input is folded to BM form first
     * ({@link #isoToBmDate}).
     */
    ...
    // ------------------------------------------------------------------
    // Denodo temporal columns <-> BM string forms
    //
    // Every archival date column is typed Date/Timestamp in the views (12 in
    // stcusttab, 20 in gld0data, 8 in stidtab, 10 in stcustlog) even though the
    // workbook documents the FORMAT as YYYYMMDD / YYYYMMDDHH24MISS. The JDBC
    // driver therefore returns ISO text ("2009-07-11", "2009-07-11 10:15:30"),
    // while the rest of the port — and the UI's formatDate/formatTimestamp —
    // expect the BM forms. These convert both ways: read with isoToBm*, and
    // convert back with bmToIso before binding a value into a predicate.
    // ------------------------------------------------------------------

    /** True for "YYYY-MM-DD..." — the shape the driver returns temporal columns in. */
    ...
    /**
     * Denodo date &rarr; {@code YYYYMMDD}. Anything not ISO (already-BM forms,
     * packed 6-char dates, blanks) passes through untouched.
     */
    public static String isoToBmDate(String raw) {
        if (raw == null) {
            return "";
        }
        String s = raw.trim();
        return looksIso(s) ? s.substring(0, 4) + s.substring(5, 7) + s.substring(8, 10) : s;
    }

    /**
     * Denodo timestamp &rarr; {@code YYYYMMDDHH24MISS}. A date with no time part
     * is padded with {@code 000000}, so the width is always 14 — the UI's
     * formatTimestamp depends on it.
     */
    public static String isoToBmTimestamp(String raw) {
        if (raw == null) {
            return "";
        }
        String s = raw.trim();
        ...
        String date = s.substring(0, 4) + s.substring(5, 7) + s.substring(8, 10);
        if (s.length() >= 19 && (s.charAt(10) == ' ' || s.charAt(10) == 'T')) {
            return date + s.substring(11, 13) + s.substring(14, 16) + s.substring(17, 19);
        }
        return date + "000000";
    }

    /**
     * BM {@code YYYYMMDD} / {@code YYYYMMDDHH24MISS} &rarr; ISO text, for binding
     * into a predicate against a Date/Timestamp column. Input that is not a BM
     * date passes through, so a value the driver already gave us in ISO stays as
     * it is.
     */
    public static String bmToIso(String bm) {
        if (bm == null) {
            return "";
        }
        String s = bm.trim();
        if (s.length() == 8 && allDigits(s, 0, 8)) {
            return s.substring(0, 4) + "-" + s.substring(4, 6) + "-" + s.substring(6, 8);
        }
        if (s.length() == 14 && allDigits(s, 0, 14)) {
            return s.substring(0, 4) + "-" + s.substring(4, 6) + "-" + s.substring(6, 8)
