...
export default function EnquirySelect({
  ...
  const handleEnquiry = () => {
    if (!selectedRow) {
      setNotice({ kind: 'warn', text: 'Empty row selected — please select a row with a value.' })
      return
    }
  ...
  const handleMore = () => {
    if (!hasMore || !onMore) {
      setNotice({ kind: 'warn', text: 'No more match found.' })
      return
    }
    setNotice(null)
    onMore()
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 rounded-2xl border border-edge bg-surface p-4 shadow-sm sm:p-5">
        ...
        <button
          type="button"
          onClick={() =>
            setNotice({ kind: 'info', text: 'Opens Customer Categorize for a new customer (to be built).' })
          }
        ...
        {/* Legacy gates update on authority ~01 (the enable itself is commented
            out in VER 4.0.5, leaving the button permanently disabled). */}
        <button
          type="button"
