    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted">
          Legacy frmInputform — on-demand statement, service 07.
        </p>
      </div>
      ...
      {unavailable ? (
        ...
        <SourceBanner title="Demo data — the online gateway is not connected">
          <p>
            The legacy reads this from <code>bmrtServer</code> over its own socket (service 07), not
            from cbcmssrv — so it has no archival view and no Denodo table. That connection is what{' '}
            <code>bank.online-db</code> (DB #2) is reserved for and it is not wired up yet, so every
            figure below is generated by <code>MockOnlineEnquiryRepository</code>.
          </p>
          <p>
            The form, validation, paging, running balance and totals are ported from the legacy and
            work unchanged once a real gateway client replaces the mock.
          </p>
        </SourceBanner>
      )}
